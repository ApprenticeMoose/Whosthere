//
//  Color.swift
//  Photopickertest
//
//  Created by Moose on 26.11.21.
//

import SwiftUI

extension Color {
    static let backgroundColor = Color("BackgroundColor")
    static let greyFourColor = Color("DetailGreyFourColor")
    static let greyOneColor = Color("DetailGreyOneColor")
    static let greyTwoColor = Color("DetailGreyTwoColor")
    static let middlegroundColor = Color("MiddlegroundColor")
    static let orangeAccentColor = Color("OrangeAccentColor")
    static let textColor = Color("TextColor")
    static let textUnchangedColor = Color("TextUnchangedColor")
    static let darkModeDisabledButtonColor = Color("DarkModeDisabledButtonColor")
    
}
